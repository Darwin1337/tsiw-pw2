...
exports.create = async (req, res) => {
    try {
        if (!req.body || !req.body.status)
            return res.status(400).json({ success: false, msg: "Status is mandatory." });

        const a = await Instance.create({
            bookId: req.params.id,
            status: req.body.status
        });


        return res.status(201).json({ success: true, msg: "Instance was registered successfully!", location: "/libraryapi/v1/instances/" + a.id });
    }
    catch (err) {
...
exports.changeInstance = async (req, res) => {
    try {
        ...
        return res.status(201).json({ success: true, msg: "Instance was registered successfully!", location: "/libraryapi/v1/instances/1" });
    }
    catch (err) {
        if (err instanceof ValidationError)
            res.status(400).json({ success: false, msg: err.errors.map(e => e.message) });
