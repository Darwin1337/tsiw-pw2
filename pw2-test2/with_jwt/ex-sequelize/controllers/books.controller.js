...
exports.findOne = async (req, res) => {
    try {
        ...
        //     return res.status(400).json({ success: false, msg: `Parâmetros incorretos` });
        // }
        // A rota nem sequer é atingida se nao tiver parametros
        ...
        book.setDataValue('nInstances', await Instance.count({ where: { bookId: req.params.id } }));

        if (book === null)
            return res.status(404).json({ success: false, msg: `Cannot find any book with ID ${req.params.id}.` });
        res.status(200).json({ success: true, books: book });
    }
    catch (err) {
        res.status(500).json({ success: false, msg: err.message || "Some error occurred while retrieving the users." });
    }
};
